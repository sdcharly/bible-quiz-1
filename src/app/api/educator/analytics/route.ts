...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const studentUsers = studentIds.length > 0 
      ? await db.select().from(user).where(inArray(user.id, studentIds))
      : [];
    
    const studentUserMap = new Map(studentUsers.map(u => [u.id, u]));
    
    // Student performance analysis (optimized without N+1)
    const studentPerformance = students.map((student) => {
        const studentUser = studentUserMap.get(student.studentId);
    ...
    logger.info(`Analytics request completed in ${duration}ms`);
